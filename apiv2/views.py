...
def get_needed_audio_descriptors(fields):
    ...
    descriptor_names += [
        descriptor_name
        for descriptor_name in settings.AVAILABLE_AUDIO_DESCRIPTORS_NAMES
        if descriptor_name in requested_fields
    ]
...
def get_needed_similarity_vectors(fields):
    ...
    similarity_spaces = [
        similarity_space_name
        for similarity_space_name in settings.SIMILARITY_SPACES_NAMES
        if f"sim_{similarity_space_name}" in requested_fields
    ]
    ...
    def get(self, request, *args, **kwargs):
        ...
        if search_form.cleaned_data["query"] is None and search_form.cleaned_data["filter"] is None:
            raise BadRequestException(
                msg="At least one request parameter from Text Search should be included in the request.", resource=self
            )
...
class SoundInstance(RetrieveAPIView):
    ...
    def get_description(cls):
        return (
            ...
            % (
                prepend_base("/docs/api"),
                "%s#sound-instance" % resources_doc_filename,
    ...
    def get_queryset(self):
        ...
        return Sound.objects.bulk_query(
            include_audio_descriptors=needs_analyzers_output,
            include_similarity_vectors=needs_similarity_vectors,
            include_remix_subqueries=include_remix_subqueries,
        )
...
class SoundAnalysisView(
    ...
    def get_description(cls):
        return (
            ...
            % (
                prepend_base("/docs/api"),
                "%s#sound-analysis" % resources_doc_filename,
...
class SimilarSounds(GenericAPIView):
    ...
    def get_description(cls):
        return (
            "Similar sounds to a given Freesound sound."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#similar-sounds" % resources_doc_filename,
    ...
    def get(self, request, *args, **kwargs):
        ...
        for i, sid in enumerate(ids):
            try:
                sound = SoundListSerializer(
                    sounds_dict[sid], score_map=id_score_map, context=self.get_serializer_context()
                ).data
                sounds.append(sound)
            except:
...
class SoundComments(ListAPIView):
    ...
    @classmethod
    def get_description(cls):
        return 'Sounds comments.<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s' % (
            prepend_base("/docs/api"),
            "%s#sound-comments" % resources_doc_filename,
...
class DownloadSound(DownloadAPIView):
    @classmethod
    def get_description(cls):
        return 'Download a sound.<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s' % (
            prepend_base("/docs/api"),
            "%s#download-sound-oauth2-required" % resources_doc_filename,
...
class UserInstance(RetrieveAPIView):
    ...
    def get_description(cls):
        return (
            ...
            % (
                prepend_base("/docs/api"),
                "%s#user-instance" % resources_doc_filename,
...
class UserSounds(ListAPIView):
    ...
    def get_description(cls):
        return (
            ...
            % (
                prepend_base("/docs/api"),
                "%s#user-sounds" % resources_doc_filename,
    ...
    def get_queryset(self):
        ...
        queryset = Sound.objects.bulk_sounds_for_user(
            user_id=user.id,
            include_audio_descriptors=needs_analyzers_output,
            include_similarity_vectors=needs_similarity_vectors,
            include_remix_subqueries=include_remix_subqueries,
...
class UserPacks(ListAPIView):
    ...
    def get_description(cls):
        return (
            "Packs created by a user."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#user-packs" % resources_doc_filename,
    ...
    def get_queryset(self):
        ...
        queryset = (
            Pack.objects.select_related("user").filter(user__username=self.kwargs["username"]).exclude(is_deleted=True)
        )
        return queryset
...
class PackInstance(RetrieveAPIView):
    ...
    def get_description(cls):
        return (
            ...
            % (
                prepend_base("/docs/api"),
                "%s#pack-instance" % resources_doc_filename,
...
class PackSounds(ListAPIView):
    ...
    def get_description(cls):
        return (
            "Sounds included in a pack."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#pack-sounds" % resources_doc_filename,
...
class DownloadPack(DownloadAPIView):
    @classmethod
    def get_description(cls):
        return 'Download a pack.<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s' % (
            prepend_base("/docs/api"),
            "%s#download-pack-oauth2-required" % resources_doc_filename,
...
class UploadSound(WriteRequiredGenericAPIView):
    ...
    def get_description(cls):
        return (
            "Upload an audiofile and (optionally) describe  it."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#upload-sound-oauth2-required" % resources_doc_filename,
        ...
        if serializer.is_valid():
            ...
            if not settings.ALLOW_WRITE_WHEN_SESSION_BASED_AUTHENTICATION and self.auth_method_name == "Session":
                ...
                if is_providing_description:
                    ...
                    return Response(
                        data={
                            "detail": "Audio file successfully uploaded and described (now pending "
...
class PendingUploads(OauthRequiredAPIView):
    ...
    def get_description(cls):
        return (
            "List of uploaded files which have not yet been described, processed or moderated."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#pending-uploads-oauth2-required" % resources_doc_filename,
...
class DescribeSound(WriteRequiredGenericAPIView):
    ...
    def get_description(cls):
        return (
            "Describe a previously uploaded sound."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#describe-sound-oauth2-required" % resources_doc_filename,
...
class EditSoundDescription(WriteRequiredGenericAPIView):
    ...
    def get_description(cls):
        return (
            "Edit the description of an existing sound."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#edit-sound-description-oauth2-required" % resources_doc_filename,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if sound.user != self.user:
            raise UnauthorizedException(
                msg="Not authorized. The sound you're trying to edit is not owned by the OAuth2 logged in user.",
                resource=self,
            )
...
class BookmarkSound(WriteRequiredGenericAPIView):
    ...
    @classmethod
    def get_description(cls):
        return 'Bookmark a sound.<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s' % (
            prepend_base("/docs/api"),
            "%s#bookmark-sound-oauth2-required" % resources_doc_filename,
...
class RateSound(WriteRequiredGenericAPIView):
    ...
    @classmethod
    def get_description(cls):
        return 'Rate a sound.<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s' % (
            prepend_base("/docs/api"),
            "%s#rate-sound-oauth2-required" % resources_doc_filename,
...
class CommentSound(WriteRequiredGenericAPIView):
    ...
    def get_description(cls):
        return (
            "Add a comment to a sound."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#comment-sound-oauth2-required" % resources_doc_filename,
...
class Me(OauthRequiredAPIView):
    ...
    def get_description(cls):
        return (
            ...
            % (
                prepend_base("/docs/api"),
                "%s#me-information-about-user-authenticated-using-oauth2-oauth2-required" % resources_doc_filename,
            )
        )
...
class MeBookmarkCategories(OauthRequiredAPIView, ListAPIView):
    ...
    def get_description(cls):
        return (
            "Bookmark categories created by the logged in user."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#me-bookmark-categories" % resources_doc_filename,
...
class MeBookmarkCategorySounds(OauthRequiredAPIView, ListAPIView):
    ...
    def get_description(cls):
        return (
            "Sounds bookmarked by a user under a particular category."
            '<br>Full documentation can be found <a href="%s/%s" target="_blank">here</a>. %s'
            % (
                prepend_base("/docs/api"),
                "%s#me-bookmark-category-sounds" % resources_doc_filename,
                get_formatted_examples_for_view("MeBookmarkCategorySounds", "apiv2-me-bookmark-category-sounds", max=5),
            )
        )
...
class FreesoundApiV2Resources(GenericAPIView):
    ...
    def get_description(cls):
        return (
            "List of resources available in the Freesound APIv2. "
        ...
        api_index = [
            ...
            },
            {
                "Other resources": OrderedDict(
                    sorted(
                        {
                            "01 Me (information about user authenticated using oauth)": prepend_base(
                                reverse("apiv2-me")
                            ),
...
def granted_permissions(request):
    ...
    return render(
        request,
        "accounts/manage_api_permissions.html",
        {
            "user": request.user,
            "tokens": tokens,
            "grants": grants,
...
def permission_granted(request):
    ...
    return render(
        request, "oauth2_provider/app_authorized.html", {"code": code, "app_name": app_name, "logout_next": logout_next}
    )
