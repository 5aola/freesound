...
def multi_email_cleanup(request):
    ...
    if not email_issues_still_valid:
        ...
        return render(
            request,
            "accounts/multi_email_cleanup.html",
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if next_param is not None:
                return JsonResponse(
                    {
                        "redirectURL": next_param + "?feedbackRegistration=1"
                        if "?" not in next_param
...
def manage_sounds(request, tab):
    def process_filter_and_sort_options(request, sort_options, tab):
        ...
        try:
            sort_by_db = [option_db_name for option_name, _, option_db_name in sort_options if option_name == sort_by][
                0
            ]
        except IndexError:
    ...
    sounds_moderation_base_qs = Sound.objects.filter(user=request.user, processing_state="OK").exclude(
        moderation_state="OK"
    )
    ...
    if tab == "pending_description":
            ...
            if packs:
                if "edit" in request.POST:
                    ...
                    messages.add_message(
                        request,
                        messages.INFO,
                        f"Successfully deleted {n_packs_deleted} pack{'s' if n_packs_deleted != 1 else ''}",
                    )
        ...
        if filter_db is not None:
            packs_base_qs = (
                packs_base_qs.annotate(search=SearchVector("name", "id", "description"))
                .filter(search=filter_db)
                .distinct()
            )
            ...
            if sounds:
                if "edit" in request.POST:
                    ...
                    messages.add_message(
                        request,
                        messages.INFO,
                        f"Successfully deleted {n_sounds_deleted} sound{'s' if n_sounds_deleted != 1 else ''}",
                    )
                    return HttpResponseRedirect(reverse("accounts-manage-sounds", args=[tab]))
                    ...
                    if n_send_to_processing != len(sounds):
                        sounds_skipped_msg_part = (
                            f" {len(sounds) - n_send_to_processing} sounds were not send to "
        ...
        if filter_db is not None:
            sounds = (
                sounds.annotate(search=SearchVector("original_filename", "id", "description", "tags__name"))
                .filter(search=filter_db)
                .distinct()
            )
...
def attribution(request):
    qs_sounds = (
        Download.objects.annotate(download_type=Value("sound", CharField()))
        .values(
            "download_type",
            "sound_id",
            "sound__user__username",
            "sound__original_filename",
            "license__name",
            "license__deed_url",
            "sound__license__name",
            "sound__license__deed_url",
            "created",
        )
        .filter(user=request.user)
    )
    qs_packs = (
        PackDownload.objects.annotate(download_type=Value("pack", CharField()))
        .values(
            "download_type",
            "pack_id",
            "pack__user__username",
            "pack__name",
            "pack__name",
            "pack__name",
            "pack__name",
...
def download_attribution(request):
    content = {"csv": "csv", "txt": "plain", "json": "json"}
                ...
                output.append(
                    {
                        ...
                        "license_name": license_with_version(
                            row["license__name"] or row["sound__license__name"],
                            row["license__deed_url"] or row["sound__license__deed_url"],
                        ),
...
def create_user_rank(uploaders, posters, commenters, weights=dict()):
    ...
    for user in user_rank:
        user_rank[user]["score"] = (
            user_rank[user]["uploads"] * upload_weight
            + user_rank[user]["posts"] * post_weight
            + user_rank[user]["comments"] * comment_weight
        )
...
def compute_charts_stats():
    ...
    latest_uploaders = (
        Sound.public.filter(created__gte=last_time).values("user").annotate(Count("id")).order_by("-id__count")
    )
    latest_posters = (
        Post.objects.filter(created__gte=last_time).values("author_id").annotate(Count("id")).order_by("-id__count")
    )
    latest_commenters = (
        Comment.objects.filter(created__gte=last_time).values("user_id").annotate(Count("id")).order_by("-id__count")
    )
    user_rank, sort_list = create_user_rank(latest_uploaders, latest_posters, latest_commenters, weights=weights)
    most_active_users = User.objects.select_related("profile").filter(
        id__in=[u[1] for u in sorted(sort_list, reverse=True)[:num_items]]
    )
    ...
    # Newest active users
    new_user_in_rank_ids = User.objects.filter(date_joined__gte=last_time, id__in=list(user_rank.keys())).values_list(
        "id", flat=True
    )
    new_user_objects = {
        user.id: user
        for user in User.objects.select_related("profile")
        .filter(date_joined__gte=last_time)
    ...
    top_recent_uploaders_by_count = (
        Sound.public.filter(created__gte=last_time)
        .values("user_id")
        .annotate(n_sounds=Count("user_id"))
        .order_by("-n_sounds")[0:num_items]
    )
    user_objects = {
        user.id: user
        for user in User.objects.select_related("profile").filter(
            id__in=[item["user_id"] for item in top_recent_uploaders_by_count]
        )
    }
    ...
    top_recent_uploaders_by_length = (
        Sound.public.filter(created__gte=last_time)
        .values("user_id")
    ...
    user_objects = {
        user.id: user
        for user in User.objects.select_related("profile").filter(
            id__in=[item["user_id"] for item in top_recent_uploaders_by_length]
        )
    }
    ...
    all_time_top_uploaders_by_count = (
        Sound.public.values("user_id").annotate(n_sounds=Count("user_id")).order_by("-n_sounds")[0:num_items]
    )
    user_objects = {
        user.id: user
        for user in User.objects.select_related("profile").filter(
            id__in=[item["user_id"] for item in all_time_top_uploaders_by_count]
        )
    }
    ...
    all_time_top_uploaders_by_length = (
        Sound.public.values("user_id").annotate(total_duration=Sum("duration")).order_by("-total_duration")[0:num_items]
    )
    user_objects = {
        user.id: user
        for user in User.objects.select_related("profile").filter(
            id__in=[item["user_id"] for item in all_time_top_uploaders_by_length]
        )
    }
...
def charts(request):
    ...
    and cache them.
    """
    tvars = caches["persistent"].get(settings.CHARTS_DATA_CACHE_KEY, None)
    if tvars is None:
        tvars = compute_charts_stats()
        caches["persistent"].set(settings.CHARTS_DATA_CACHE_KEY, tvars, 60 * 60 * 24)
...
def bulk_describe(request, bulk_id):
    if not request.user.profile.can_do_bulk_upload():
        messages.add_message(
            request,
            messages.INFO,
            "Your user does not have permission to use the bulk describe "
...
def delete(request):
    ...
    if request.method == "POST":
        ...
        if not form.is_valid():
            ...
            recent_pending_deletion_requests_exist = (
                UserDeletionRequest.objects.filter(user_to_id=request.user.id, last_updated__gt=cutoff_date)
                .filter(status=UserDeletionRequest.DELETION_REQUEST_STATUS_DELETION_TRIGGERED)
                .exists()
            )
            if recent_pending_deletion_requests_exist:
                ...
                delete_sounds = form.cleaned_data["delete_sounds"] == "delete_sounds"
                delete_action = (
                    DELETE_USER_DELETE_SOUNDS_ACTION_NAME if delete_sounds else DELETE_USER_KEEP_SOUNDS_ACTION_NAME
                )
...
def flag_user(request, username):
    if request.POST:
        ...
        if reports_count != previous_reports_count and (
            reports_count == settings.USERFLAG_THRESHOLD_FOR_NOTIFICATION
            or reports_count == settings.USERFLAG_THRESHOLD_FOR_AUTOMATIC_BLOCKING
        ):
