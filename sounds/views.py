...
def sound(request, username, sound_id):
    try:
        sound = (
            Sound.objects.prefetch_related("tags")
    ...
    qs = Comment.objects.select_related("user", "user__profile").filter(sound_id=sound_id)
...
def after_download_modal(request):
    ...
    if should_show_modal:
        return render(
            request,
            "donations/modal_after_download_donation_request.html",
            {"donation_amount_request_param": settings.DONATION_AMOUNT_REQUEST_PARAM},
        )
    else:
        return HttpResponse()
...
def edit_and_describe_sounds_helper(request, describing=False, session_key_prefix=""):
    ...
    def create_sounds(request, forms):
        ...
        for form in forms:
            ...
            if not form.cleaned_data.get("remove_geotag") and form.cleaned_data.get(
                "lat"
            ):  # if 'lat' is in data, we assume other fields are too
                geotag = "%s,%s,%d" % (
                    form.cleaned_data.get("lat"),
                    form.cleaned_data.get("lon"),
                    form.cleaned_data.get("zoom"),
                )
            ...
            try:
                ...
                if user.profile.is_whitelisted:
                    ...
                    messages.add_message(
                        request,
                        messages.INFO,
                        'File <a href="{}">{}</a> has been described and is now awaiting processing and moderation.'.format(
                            sound.get_absolute_url(), sound.original_filename
                        ),
                    )
        ...
        if data["remove_geotag"]:
            ...
            if data["lat"] and data["lon"] and data["zoom"]:
                if hasattr(sound, "geotag"):
                    ...
                    GeoTag.objects.create(sound=sound, lat=data["lat"], lon=data["lon"], zoom=data["zoom"])
    ...
    len_original_describe_edit_sounds = (
        request.session.get(f"{session_key_prefix}-len_original_describe_sounds", 0)
        if describing
        else request.session.get(f"{session_key_prefix}-len_original_edit_sounds", 0)
    )
...
def display_sound_wrapper(request, username, sound_id):
    ...
    if sound_obj is not None:
        is_explicit = sound_obj.is_explicit and (not request.user.is_authenticated or not request.user.profile.is_adult)
    tvars = {
        "sound_id": sound_id,
        "sound": sound_obj,
        "request": request,
