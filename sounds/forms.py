...
def _pack_form_clean_pack_helper(cleaned_data):
    if (
        "pack" not in cleaned_data
        or cleaned_data["pack"] == ""
        or str(cleaned_data["pack"]) == PackForm.NO_PACK_CHOICE_VALUE
    ):
        # No pack selected
        return None
...
class PackForm(forms.Form):
    ...
    def __init__(self, pack_choices, *args, **kwargs):
        ...
        self.fields["pack"].choices = [
            (self.NO_PACK_CHOICE_VALUE, "--- No pack ---"),
...
class LicenseForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if hide_old_license_versions:
            new_qs = License.objects.filter(
                Q(name__istartswith="Attribution") | Q(name__istartswith="Creative")
            ).exclude(deed_url__contains="3.0")
...
class SoundEditAndDescribeForm(forms.Form):
    ...
    name = forms.CharField(
        max_length=512, min_length=5, widget=forms.TextInput(attrs={"size": 65, "class": "inputText"})
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user_packs:
            ...
            user_packs = []
        self.fields["pack"].choices = [
            (PackForm.NO_PACK_CHOICE_VALUE, "--- No pack ---"),
...
class SoundCSVDescriptionForm(SoundEditAndDescribeForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["bst_category"].error_messages.update(
            {
                "invalid_choice": "Please use a valid BST subcategory ID. See bulk describe instructions for more details."
            }
        )
